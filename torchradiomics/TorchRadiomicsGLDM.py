...
class TorchRadiomicsGLDM(TorchRadiomicsBase):
  r"""
  ...
  def __init__(self, inputImage, inputMask, **kwargs):
    super(TorchRadiomicsGLDM, self).__init__(inputImage, inputMask, **kwargs)
  ...
  def _calculateMatrix(self, voxelCoordinates=None):
    ...
    P_gldm = P_gldm[:, GrayLevels - 1, :]

    jvector = torch.arange(1, P_gldm.shape[2] + 1, dtype=self.dtype, device=self.device)
    ...
    P_gldm = self.delete(P_gldm, torch.where(empty_sizes == 0), 2)
    jvector = self.delete(jvector, torch.where(empty_sizes == 0), 0)
    pd = self.delete(pd, torch.where(empty_sizes == 0), 1)

    Nz = torch.sum(pd, 1)  # Nz per kernel, shape (Nv, )
    ...
    self.coefficients['ivector'] = self.tensor(self.coefficients['grayLevels'])
  ...
  def getSmallDependenceEmphasisFeatureValue(self):
    ...
    **1. Small Dependence Emphasis (SDE)**
    ...
    sde = torch.sum(pd / (jvector[None, :] ** 2), 1) / Nz
    return sde.cpu().numpy()
  ...
  def getLargeDependenceEmphasisFeatureValue(self):
    ...
    **2. Large Dependence Emphasis (LDE)**
    ...
    lre = torch.sum(pd * (jvector[None, :] ** 2), 1) / Nz
    return lre.cpu().numpy()
  ...
  def getGrayLevelNonUniformityFeatureValue(self):
    ...
    **3. Gray Level Non-Uniformity (GLN)**
    ...
    gln = torch.sum(pg ** 2, 1) / Nz
    return gln.cpu().numpy()
  ...
  def getDependenceNonUniformityNormalizedFeatureValue(self):
    ...
    **5. Dependence Non-Uniformity Normalized (DNN)**
    ...
    dnn = torch.sum(pd ** 2, 1) / Nz ** 2
    return dnn.cpu().numpy()
  ...
  def getDependenceVarianceFeatureValue(self):
    ...
    **7. Dependence Variance (DV)**
    ...
    u_j = torch.sum(pd * jvector[None, :], 1, keepdims=True)
    dv = torch.sum(pd * (jvector[None, :] - u_j) ** 2, 1)
    return dv.cpu().numpy()
  ...
  def getDependenceEntropyFeatureValue(self):
    ...
    **8. Dependence Entropy (DE)**
    ...
    return -torch.sum(p_gldm * torch.log2(p_gldm + eps), (1, 2)).cpu().numpy()
  ...
  def getLowGrayLevelEmphasisFeatureValue(self):
    ...
    **9. Low Gray Level Emphasis (LGLE)**
    ...
    lgle = torch.sum(pg / (ivector[None, :] ** 2), 1) / Nz
    return lgle.cpu().numpy()
  ...
  def getHighGrayLevelEmphasisFeatureValue(self):
    ...
    **10. High Gray Level Emphasis (HGLE)**
    ...
    hgle = torch.sum(pg * (ivector[None, :] ** 2), 1) / Nz
    return hgle.cpu().numpy()
  ...
  def getSmallDependenceLowGrayLevelEmphasisFeatureValue(self):
    ...
    **11. Small Dependence Low Gray Level Emphasis (SDLGLE)**
    ...
    sdlgle = torch.sum(self.P_gldm / ((ivector[None, :, None] ** 2) * (jvector[None, None, :] ** 2)), (1, 2)) / Nz
    return sdlgle.cpu().numpy()
  ...
  def getSmallDependenceHighGrayLevelEmphasisFeatureValue(self):
    ...
    **12. Small Dependence High Gray Level Emphasis (SDHGLE)**
    ...
    sdhgle = torch.sum(self.P_gldm * (ivector[None, :, None] ** 2) / (jvector[None, None, :] ** 2), (1, 2)) / Nz
    return sdhgle.cpu().numpy()
  ...
  def getLargeDependenceLowGrayLevelEmphasisFeatureValue(self):
    ...
    **13. Large Dependence Low Gray Level Emphasis (LDLGLE)**
    ...
    ldlgle = torch.sum(self.P_gldm * (jvector[None, None, :] ** 2) / (ivector[None, :, None] ** 2), (1, 2)) / Nz
    return ldlgle.cpu().numpy()
  ...
  def getLargeDependenceHighGrayLevelEmphasisFeatureValue(self):
    ...
    **14. Large Dependence High Gray Level Emphasis (LDHGLE)**
    ...
    ldhgle = torch.sum(self.P_gldm * ((jvector[None, None, :] ** 2) * (ivector[None, :, None] ** 2)), (1, 2)) / Nz
    return ldhgle.cpu().numpy()
