...
class TorchRadiomicsGLSZM(TorchRadiomicsBase):
  r"""
  ...
  def __init__(self, inputImage, inputMask, **kwargs):
    super(TorchRadiomicsGLSZM, self).__init__(inputImage, inputMask, **kwargs)
  ...
  def _calculateCoefficients(self):
    ...
    ps = torch.sum(self.P_glszm, 1)  # shape (Nvox, Ns)
    pg = torch.sum(self.P_glszm, 2)  # shape (Nvox, Ng)

    ivector = self.tensor(self.coefficients['grayLevels'])  # shape (Ng,)
    jvector = torch.arange(1, self.P_glszm.shape[2] + 1, dtype=self.dtype, device=self.device)  # shape (Ns,)

    # Get the number of zones in this GLSZM
    Nz = torch.sum(self.P_glszm, (1, 2))  # shape (Nvox,)
    ...
    Np = torch.sum(ps * jvector[None, :], 1)  # shape (Nvox, )
    Np[Np == 0] = 1

    # Delete columns that specify zone sizes not present in the ROI
    emptyZoneSizes = torch.where(torch.sum(ps, 0) == 0)
    self.P_glszm = self.delete(self.P_glszm, emptyZoneSizes, 2)
    jvector = self.delete(jvector, emptyZoneSizes, 0)
    ps = self.delete(ps, emptyZoneSizes, 1)
  ...
  def getSmallAreaEmphasisFeatureValue(self):
    ...
    **1. Small Area Emphasis (SAE)**
    ...
    sae = torch.sum(ps / (jvector[None, :] ** 2), 1) / Nz
    return sae.cpu().numpy()
  ...
  def getLargeAreaEmphasisFeatureValue(self):
    ...
    **2. Large Area Emphasis (LAE)**
    ...
    lae = torch.sum(ps * (jvector[None, :] ** 2), 1) / Nz
    return lae.cpu().numpy()
  ...
  def getGrayLevelNonUniformityNormalizedFeatureValue(self):
    ...
    **4. Gray Level Non-Uniformity Normalized (GLNN)**
    ...
    ivn = torch.sum(pg ** 2, 1) / Nz ** 2
    return ivn.cpu().numpy()
  ...
  def getSizeZoneNonUniformityFeatureValue(self):
    ...
    **5. Size-Zone Non-Uniformity (SZN)**
    ...
    szv = torch.sum(ps ** 2, 1) / Nz
    return szv.cpu().numpy()
  ...
  def getSizeZoneNonUniformityNormalizedFeatureValue(self):
    ...
    **6. Size-Zone Non-Uniformity Normalized (SZNN)**
    ...
    szvn = torch.sum(ps ** 2, 1) / Nz ** 2
    return szvn.cpu().numpy()
  ...
  def getGrayLevelVarianceFeatureValue(self):
    ...
    **8. Gray Level Variance (GLV)**
    ...
    u_i = torch.sum(pg * ivector[None, :], 1, keepdims=True)
    glv = torch.sum(pg * (ivector[None, :] - u_i) ** 2, 1)
    return glv.cpu().numpy()
  ...
  def getZoneVarianceFeatureValue(self):
    ...
    **9. Zone Variance (ZV)**
    ...
    u_j = torch.sum(ps * jvector[None, :], 1, keepdims=True)
    zv = torch.sum(ps * (jvector[None, :] - u_j) ** 2, 1)
    return zv.cpu().numpy()
  ...
  def getZoneEntropyFeatureValue(self):
    ...
    **10. Zone Entropy (ZE)**
    ...
    ze = -torch.sum(p_glszm * torch.log2(p_glszm + eps), (1, 2))
    return ze.cpu().numpy()
  ...
  def getLowGrayLevelZoneEmphasisFeatureValue(self):
    ...
    **11. Low Gray Level Zone Emphasis (LGLZE)**
    ...
    lie = torch.sum(pg / (ivector[None, :] ** 2), 1) / Nz
    return lie.cpu().numpy()
  ...
  def getHighGrayLevelZoneEmphasisFeatureValue(self):
    ...
    **12. High Gray Level Zone Emphasis (HGLZE)**
    ...
    hie = torch.sum(pg * (ivector[None, :] ** 2), 1) / Nz
    return hie.cpu().numpy()
  ...
  def getSmallAreaLowGrayLevelEmphasisFeatureValue(self):
    ...
    **13. Small Area Low Gray Level Emphasis (SALGLE)**
    ...
    lisae = torch.sum(self.P_glszm / ((ivector[None, :, None] ** 2) * (jvector[None, None, :] ** 2)), (1, 2)) / Nz
    return lisae.cpu().numpy()
  ...
  def getSmallAreaHighGrayLevelEmphasisFeatureValue(self):
    ...
    **14. Small Area High Gray Level Emphasis (SAHGLE)**
    ...
    hisae = torch.sum(self.P_glszm * (ivector[None, :, None] ** 2) / (jvector[None, None, :] ** 2), (1, 2)) / Nz
    return hisae.cpu().numpy()
  ...
  def getLargeAreaLowGrayLevelEmphasisFeatureValue(self):
    ...
    **15. Large Area Low Gray Level Emphasis (LALGLE)**
    ...
    lilae = torch.sum(self.P_glszm * (jvector[None, None, :] ** 2) / (ivector[None, :, None] ** 2), (1, 2)) / Nz
    return lilae.cpu().numpy()
  ...
  def getLargeAreaHighGrayLevelEmphasisFeatureValue(self):
    ...
    **16. Large Area High Gray Level Emphasis (LAHGLE)**
    ...
    hilae = torch.sum(self.P_glszm * (ivector[None, :, None] ** 2) * (jvector[None, None, :] ** 2), (1, 2)) / Nz
    return hilae.cpu().numpy()
